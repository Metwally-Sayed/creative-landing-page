...
class CommonClass {
  ...
  init(container: HTMLElement) {
    ...
    // We'll append it manually in the effect to avoid duplication if strictly managed, 
    // but here init is called from inside an effect.
    if (container) {
  ...
  resize() {
    ...
    console.log('LiquidEther resize:', this.width, this.height);
...
class ManagerClass {
  ...
  constructor() {
    ...
    // @ts-ignore
    Mouse.onInteract = this.onInteract;
  }
}
...
class ExternalForce extends ShaderPass {
  ...
  update(props?: any) {
    ...
    if (
      this.mouse &&
      (this.mouse.material instanceof THREE.RawShaderMaterial ||
        this.mouse.material instanceof THREE.ShaderMaterial)
    ) {
      const uniforms = this.mouse.material.uniforms as {
...
class Simulation {
  ...
  calcSize() {
    ...
    this.boundarySpace.set(px_x, px_y);
  }
  resize() {
    this.calcSize();
    for (let key in this.fbos) {
  ...
  update() {
    ...
    // @ts-ignore
    this.poisson.update({ iterations: this.options.iterations_poisson });
    // @ts-ignore
    this.pressure.update({ vel: this.fbos.vel_1, pressure: this.fbos.pressure_0 });
  }
}
...
export default function LiquidEther({
  ...
  resolution = 0.5,
  isBounce = false,
  // Default to light grainy gradient colors: White, Pale Blue, Soft Orange
  colors = ['#ffffff', '#e0f2fe', '#ffedd5'],
  style = {},
  className = '',
  autoDemo = true,
  autoSpeed = 0.5,
  ...
  useEffect(() => {
    ...
    if (mountRef.current) {
      console.log('Liquid Ether mounting...');
      Common.init(mountRef.current);
      Mouse.init(mountRef.current);
    }

    const SimulationInstance = new Simulation({
    ...
    const GenerateOutput = new ShaderPass({
      material: {
        vertexShader: face_vert,
        fragmentShader: color_frag,
        uniforms: {
          velocity: { value: SimulationInstance.fbos.vel_0.texture },
    ...
    // @ts-ignore
    if (mountRef.current) intersectionObserverRef.current.observe(mountRef.current);
    ...
    return () => {
      ...
      if (Common.renderer && mountRef.current) {
        // Double check it is a child before removing
        try {
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          backgroundImage: `url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/200 200'%3E%3Cfilter id='noiseFilter'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.65' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noiseFilter)'/%3E%3C/svg%3E")`,
        }}
      />
    </div>
  );
